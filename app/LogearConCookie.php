...
session_start();
	...
	$sql = "SELECT u.password password, u.verificado, c.id IDCliente, CONCAT(c.nombres, ' ', c.apellidos) NombreCliente, c.dpi DPI 
	...
	if($contar == 1){
		...
		$sql2 = "SELECT u.password password, u.verificado FROM usuario u WHERE u.f_tipousuario = 1 AND u.email = '".$correo."';";
		...
		foreach ($Reg as $registro) {
			...
			if(password_verify($pass, $PassReal)){
				...
				if($Verificado == 1){//Vemos si el correo esta verificado
					...
					//Creando la sesion
					CrearSesion($IDCliente, $Nombre, $DPI, $email);
					...
					header('Location: ../indexLogeo.php'); //envia a la cuenta
				}else{
					//El correo no ha sido verificado
					header('Location: ../Registro/AnuncioEmailNoVerificado.php?email=' . $email); //envia a la cuenta
				}
			}else{
